<template>
	<view  class="jsfun-record" @tap="showPicker">
	 ...
	 <!-- 多选控件 -->
	 <view class="conbox record" :class="{'pickerShow':isShow}" catchtouchmove="true">
		...
		<view class="record-box">
			<span class="stop" @tap.stop="stopVoice" v-if="voicePath && playing==1"></span>
			<span class="paly" @tap.stop="playVoice" v-if="voicePath && playing==0"></span>
			<canvas class="canvas" canvas-id="canvas">
				<span class="recording" @touchstart="start" @longpress="record" @touchend="end"></span>
			</canvas>
			<span class="confirm" @tap.stop="okClick" v-if="voicePath"></span>
		</view>
	...
	export default {
		...
		methods: {
			...
			record: function() {
				...
				let centerX = _this.tempw/2-1;
				let centerY = _this.temph/2-1;
