...
<script>	
	...
	export default {
		onLoad:function(option){
			_self = this;
			this.tabCurrentIndex = option.index;
			this.getHeadType();
		},
		data() {
			return {
		...
		methods:{
			getHeadType:function(){
				_self.$qyc.request(
					"/f/mp/mplogin/findDiclable", {dicType:'wx_supervise_type'},
					function(res) {
						_self.tabs = res.data;
			...
			getArticleList:function(){
				_self.$qyc.request(
					"/f/mp/mplogin/findArticleList", {newsType:2},
					function(res) {
						data = res.data;
						_self.newsAll = [data.filter(function (e) { return e.typeSupervise == _self.tabs[_self.tabCurrentIndex].dictValue; })];
			...
			tabChange: function (e) {
				...
				_self.newsAll[index] = data.filter(function (e) { return e.typeSupervise == value; });
